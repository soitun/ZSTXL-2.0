...
@interface ShaixuanPharViewController ()

@end

@implementation ShaixuanPharViewController
...
- (void)selectFinished
{
    PERFORM_SELECTOR_WITH_OBJECT(self.delegate, @selector(shaixuanPharSelectFinish:), self.selectArray);
    [self popVC:nil];
}
...
- (void)requestPharData
{
    ...
    [DreamFactoryClient getWithURLParameters:paraDict success:^(NSDictionary *json) {
        ...
        [pharArray enumerateObjectsUsingBlock:^(NSDictionary *pharDict, NSUInteger idx, BOOL *stop) {
            [self.dataSourceArray addObject:pharDict];
        }];
