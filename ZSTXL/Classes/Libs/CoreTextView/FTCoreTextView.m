...
- (CGFloat)heighWithImage:(UIImage *)img {
    if (img.size.height > 450) {
        return img.size.height/2.0f;
    }
    return img.size.height;
}

- (CGFloat)widthWithImage:(UIImage *)img {
    if (img.size.width > 600) {
        return img.size.width/2.0f;
    }
...
//                    UIImage *img = [UIImage imageNamed:elementContent];
                    UIImage *img = nil;
                    if ([self.delegate respondsToSelector:@selector(imageToShowWithName:)]) {
                        img = [self.delegate imageToShowWithName:elementContent];
                        img = [img scaleToFitSize:CGSizeMake(300, 250)];
                    }
                    
                    if (img) {
                        NSString *lines = @"\n";
                        float leading = [self heighWithImage:img];
...
{
//    CGFloat widthValue = 0.0f;
//    CFNumberRef strokeWidth = CFNumberCreate(NULL,kCFNumberFloatType,&widthValue);
//    
//    [*attributedString addAttribute:(id)(kCTStrokeWidthAttributeName) 
...
//                              range:styleRange];
	...
	CGFloat paragraphFirstLineHeadIntent = 35; //style.paragraphInset.left;
...
//			UIImage *img = [UIImage imageNamed:imageNode.imageName];
            UIImage *img = nil;
            if ([self.delegate respondsToSelector:@selector(imageToShowWithName:)]) {
                img = [self.delegate imageToShowWithName:imageNode.imageName];
                img = [img scaleToFitSize:CGSizeMake(300, 225)];
            }
            
			if (img) {
				int x = 0;
				if (alignment == kCTRightTextAlignment) x = (self.frame.size.width - [self widthWithImage:img]);
				if (alignment == kCTCenterTextAlignment) x = ((self.frame.size.width - [self widthWithImage:img])/2);
				CGRect frame = CGRectMake(x, (lineFrame.origin.y - [self heighWithImage:img]), [self widthWithImage:img], [self heighWithImage:img]);
