...
- (id)initWithItemsArray:(NSArray *)array
{
    ...
    if (self) {
        ...
        UIImageView *imageView = [[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 320, 36)] autorelease];
        ...
        for (int i = 0; i < arrayCount; i++) {
            ...
            button.frame = CGRectMake(i*320.0/arrayCount, 0, width, 36);
            [self addSubview:button];
            [self.buttonsArray addObject:button];
            if (i == 0) {
                self.bgSlideView.frame = CGRectMake(button.frame.origin.x, 0, button.frame.size.width, 36) ;
            }
        }
